// import {spellsArr, randomEffects} from "./data"

const slevel = document.getElementById("spellLevel");
...
function getByLevel(obj, lvl) {
  if (obj.spell_level == lvl) {
    return obj;
  }
}

function getRSpell(slotsArr){
    let count = 0;
    let selectLvl = 0;
    ...
    if (count == 0) {
        console.log("you are out of spell slots");
        return selectLvl;
    }
    selectLvl = Math.floor(Math.random() * (slotsArr.length + 1)); 
    if (selectLvl == 0){
        return selectLvl
    }
    if (slotsArr[selectLvl-1] == 0){
        return getRSpell(slotsArr);
...
const cantrips = spellsArr.filter((ob) => getByLevel(ob, 0));
const lvl1 = spellsArr.filter((ob) => getByLevel(ob, 1));
const lvl2 = spellsArr.filter((ob) => getByLevel(ob, 2));
const lvl3 = spellsArr.filter((ob) => getByLevel(ob, 3));
const lvl4 = spellsArr.filter((ob) => getByLevel(ob, 4));
const lvl5 = spellsArr.filter((ob) => getByLevel(ob, 5));
const lvl6 = spellsArr.filter((ob) => getByLevel(ob, 6));
const lvl7 = spellsArr.filter((ob) => getByLevel(ob, 7));
const lvl8 = spellsArr.filter((ob) => getByLevel(ob, 8));
const lvl9 = spellsArr.filter((ob) => getByLevel(ob, 9));
...
castButton.addEventListener("click", function a(e) {
  randomP.textContent = "";
  let firstRoll = Math.floor(Math.random() * 12);
  if (firstRoll == 1) {
    let randomEfNum = Math.floor(Math.random() * randomEffects.length);
    randomP.textContent = randomEffects[randomEfNum];
  }
  let selectLvl = getRSpell(spellslots);
  if (selectLvl == 0) {
    let rspell = Math.floor(Math.random() * cantrips.length);
    try {
    // const sText = ` you cast ${cantrips[rspell].name}, which is a cantrip.
        
    //     "${cantrips[rspell].desc}"
    //     ${cantrips[rspell].higher_level}
    //     it has a range of ${cantrips[rspell].range} , takes ${cantrips[rspell].casting_time} to cast,
    //     it is/lasts ${cantrips[rspell].duration}
    //     does it require concentration? ${cantrips[rspell].concentration}

    //     it requires the following components ${cantrips[rspell].components}
        
    //         `;

    // ptag.textContent = sText;
    slevel.textContent = `you cast ${cantrips[rspell].name}, which is a cantrip.`;
    desc.textContent = `${cantrips[rspell].desc}`;
    hlevel.textContent = `${cantrips[rspell].higher_level}`;
    range.textContent = `it has a range of ${cantrips[rspell].range}`;
    ctime.textContent = `it takes ${cantrips[rspell].casting_time} to cast`; 
    duration.textContent = `it is/lasts ${cantrips[rspell].duration}`;
    concentration.textContent = (cantrips[rspell].requires_concentration) ?  "the cantrip requires concentration" : "the cantrip doesnt requires concentration" ;
    components.textContent = `it requires the following components: ${cantrips[rspell].components}`
    ...
    return;
        ...
        console.log(cantrips[rspell])
    }
  }
  let availableSpells = [];

  for (let index = 1; index <= selectLvl; index++) {
    if (spellslots[index - 1] <= 0) {
      continue;
    }
    const element = allSpellsArr[index];
    availableSpells.push(...element);
  }



  let rspell = Math.floor(Math.random() * availableSpells.length);
  spellslots[selectLvl-1]--;

  try {
  // const sText = ` you cast ${availableSpells[rspell].name} (a level ${availableSpells[rspell].spell_level} spell) at level ${selectLvl}\n
        
  //       description "${availableSpells[rspell].desc}"\n
  //       at higher level --- ${availableSpells[rspell].higher_level}\n
  //       it has a range of ${availableSpells[rspell].range} , takes ${availableSpells[rspell].casting_time} to cast,
  //       it is/lasts ${availableSpells[rspell].duration}.\n
  //       does it require concentration? ${availableSpells[rspell].concentration}\n

  //       it requires the following components ${availableSpells[rspell].components}
  ...
  slevel.textContent = `you cast ${availableSpells[rspell].name} (a level ${availableSpells[rspell].spell_level} spell) at level ${selectLvl}`;
  desc.textContent = `${availableSpells[rspell].desc}`;
  hlevel.textContent = `${availableSpells[rspell].higher_level}`;
  range.textContent = `it has a range of ${availableSpells[rspell].range}`;
  ctime.textContent = `it takes ${availableSpells[rspell].casting_time} to cast`; 
  duration.textContent = `it is/lasts ${availableSpells[rspell].duration}`;
  concentration.textContent = (availableSpells[rspell].requires_concentration) ?  "the spell requires concentration" : "the spell doesnt requires concentration" ;
  components.textContent = `it requires the following components: ${availableSpells[rspell].components}`
...
sleepButton.addEventListener("click", function e(e){
  ...
  for (let i = 0; i < allSpellsArr.length; i++){
    console.log(allSpellsArr[i].length)
    console.log(allSpellsArr[i]);  }
})